...
public class Neuron {
	...
	Random rand = new Random(1); //Initialize random number generator with seed of 1
	public Neuron(int inputNum){
		//Constructor Initializes synaptic weights
		synapticWeights = new double[inputNum]; //Make sure that we have as many weights as we have inputs
	...
	public void train(int[][] trainingDataIn, int[] trainingDataOut, int numberOfTrainingIterations){
		...
		//Repeat for the number of training iterations to calculate the best weights possible
		//System.out.printf("%27s|%27s|%27s|%21s|","Predictions","Errors","Adjustments","New Weights");
		for(int i = 0; i < numberOfTrainingIterations; i++){
			for(int y = 0; y < trainingDataOut.length; y++){
				//Forward Propagate (Just predict an output based on the current weights and training input)
			...
			//Now backpropagate. Basically take derivative of sigmoid at the output and multiply it by the error
			for(int y = 0; y < error.length; y++){
				adjustment[y] = error[y] * sigmoidDeriv(predictions[y]);
				//System.out.printf("%5.4f ,",adjustment[y]);
			}
			//Last step; update synaptic weights by multiplying the input array by the adjustment array and adding them to the synaptic weights
	...
	public double sigmoidDeriv(int x){ //Use derivative of sigmoid function to backpropagate.
		return sigmoid(x) * (1 - sigmoid(x));
	}
	public double sigmoidDeriv(double x){ //Use derivative of sigmoid function to backpropagate.
		return sigmoid(x) * (1 - sigmoid(x));
	}
	
	//Linear Algebra Methods
	public double dot(int[] input, double[] weight){ //Dot product of two 1xN arrays
		double sum = 0;
